// server.js — Sphere: круглые "сферы", панель-эмулятор, safe-area header, .mobileconfig
...
const server = http.createServer(async (req,res)=>{
  ...
  if(req.url==="/app"){
    const html = [
      ...
      "  .top{position:fixed;left:12px;right:12px;top:calc(env(safe-area-inset-top) + 8px);height:56px;display:flex;gap:10px;align-items:center;justify-content:space-between;padding:10px 12px;border:1px solid var(--line);border-radius:14px;background:rgba(12,13,14,.85);backdrop-filter:blur(8px);z-index:20}",
      ...
      "  .content{position:absolute;left:0;right:0;top:calc(env(safe-area-inset-top) + 56px + 20px);bottom:0;padding:12px}",
      "  .card{border:1px solid var(--line);background:var(--panel);border-radius:16px;padding:14px}",
      "  .row{display:flex;align-items:center;justify-content:space-between}",
      "  .muted{color:var(--muted)}",
      "  .orbit{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-top:12px}",
      "  .cell{display:flex;justify-content:center}",
      "  .sphere{width:76px;height:76px;border-radius:50%;border:1px solid #2a2e33;background:radial-gradient(60% 60% at 40% 35%, rgba(200,208,216,0.18), rgba(80,84,92,0.08) 60%, rgba(0,0,0,0.0) 61%);display:flex;align-items:center;justify-content:center;color:#d9dde2;letter-spacing:.08em;box-shadow:inset 0 0 0 1px rgba(255,255,255,.02)}",
      "  .sphere:active{filter:brightness(1.1)}",
      "  .panel{position:fixed;left:0;right:0;bottom:0;top:calc(env(safe-area-inset-top) + 56px + 20px);background:#0b0d10;border-top:1px solid var(--line);z-index:30;display:none;flex-direction:column}",
      ...
      "  .btn{padding:8px 10px;border:1px solid var(--line);border-radius:10px;background:var(--panel);color:var(--text)}",
      "  .panel-frame{flex:1;border:0;background:#000}",
      "  .panel-msg{flex:1;display:flex;align-items:center;justify-content:center;color:var(--muted);text-align:center;padding:24px}",
      "</style>",
      "</head>",
      "<body>",
      "  <div class=\"top\">",
      ...
      "        <button class=\"btn\" id=\"openExternal\">Открыть через прокси</button>",
  ...
  if(req.url==="/app.js"){
    const js = [
      ...
      "  var APPS=[",
      "    {id:'add',name:'+',url:'https://m.wikipedia.org/' ,allow:true},",
      "    {id:'yt', name:'YouTube', url:'https://m.youtube.com/', allow:false},",
      "    {id:'x',  name:'X',       url:'https://m.twitter.com/', allow:false},",
      "    {id:'sr', name:'Search',  url:'https://lite.bing.com/' ,allow:true}",
      "  ];",
      ...
      "  function appCell(a){ var d=document.createElement('div'); d.className='cell'; var b=document.createElement('button'); b.type='button'; b.className='sphere'; b.textContent=a.name; b.onclick=function(){ openApp(a); }; d.appendChild(b); return d; }",
      "  function openApp(a){",
      "    var panel=el('panel'), fr=el('panelFrame'), msg=el('panelMsg');",
      "    el('panelTitle').textContent=a.name; panel.classList.add('open'); msg.style.display='none'; fr.style.display='block';",
      "    var target=a.url;",
      "    if(state.proxy){ target=PROXY_BASE+'?url='+encodeURIComponent(a.url); }",
      "    if(a.allow){ fr.src=target; }",
      "    else{",
      "      fr.style.display='none';",
      "      msg.style.display='flex';",
      "      msg.textContent='Этот сервис запрещает встраивание в iFrame. Нажми «Открыть через прокси», чтобы открыть в новой вкладке (дальше демонстрируешь IP через прокси).';",
      "    }",
      "    el('openExternal').onclick=function(){ window.open(PROXY_BASE+'?url='+encodeURIComponent(a.url),'_blank'); };",
      "  }",
      "  function closePanel(){ el('panel').classList.remove('open'); el('panelFrame').src='about:blank'; }",
      "  function renderUnauthed(){",
      "    var root=el('appRoot');",
      "    root.innerHTML=['<div class=\"card\">',",
      "      '<div class=\"row\"><div><div style=\"font-weight:700\">Вход в Sphere</div><div class=\"muted\">Телеграм-логин (опционально) и проверка прокси.</div></div>',",
      "      '<button class=\"btn\" onclick=\"testProxy()\">Показать IP через прокси</button></div>',",
      "      '<div id=\"tg-root\" style=\"margin-top:10px\"></div>',",
      "      '<div id=\"tg-missing\" class=\"muted\" style=\"display:none\">TG-бот не настроен</div>',",
      "      '</div>'].join('');",
      "    if(BOT_NAME){ var s=document.createElement('script'); s.async=true; s.src='https://telegram.org/js/telegram-widget.js?22'; s.setAttribute('data-telegram-login',BOT_NAME); s.setAttribute('data-size','large'); s.setAttribute('data-auth-url','/auth/telegram'); s.setAttribute('data-request-access','write'); el('tg-root').appendChild(s);} else { el('tg-missing').style.display='block'; }",
      "  }",
      ...
      "    var root=el('appRoot'); var name=(state.me&&state.me.name)||(state.me&&state.me.first_name)||'User';",
      "    root.innerHTML=['<div class=\"card\">',",
      "      '<div class=\"row\"><div><div style=\"font-weight:700\">Привет, '+name+'</div><div class=\"muted\">Все запросы из Sphere можно пускать через наш прокси.</div></div>',",
      "      '<button class=\"btn\" onclick=\"testProxy()\">IP через прокси</button></div>',",
      "      '<div class=\"orbit\" id=\"orbit\"></div>',",
      "     '</div>'].join('');",
      "    var orb=el('orbit'); APPS.forEach(function(a){ orb.appendChild(appCell(a)); });",
      "  }",
      "  async function boot(){",
      "    setProxy(true); document.getElementById('proxyToggle').onclick=function(){ setProxy(!state.proxy); };",
      "    document.getElementById('closePanel').onclick=closePanel;",
      "    state.me=await apiMe();",
      "    if(!state.me||!state.me.id) renderUnauthed(); else if(!state.me.name) renderNamePrompt(); else renderAuthed();",
      "  }",
      "  window.testProxy=testProxy; window.loginName=loginName;",
      "  boot();",
      "})();"
    ].join("\n");
