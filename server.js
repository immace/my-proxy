...
// - /app (+ static /public) — UI
// - /auth/telegram, /api/me, /api/name — логин/профиль
// - /api/spheres (GET/POST/DELETE) — сферы пользователя (в памяти процесса)
// - /m?url=... — обратный прокси в iframe через UPSTREAM_PROXY, снимаем XFO/CSP
// - /Sphere.mobileconfig — профиль ярлыка iOS
// - /healthz
...
const { ProxyAgent } = require("proxy-agent"); // ВАЖНО: именно так импорт

// ====== конфиг/ENV ===========================================================
const PORT = process.env.PORT || 8080;
const STATIC_DIR = path.join(__dirname, "public");

// твой платный HTTP-прокси (можно оставить жёстко, но лучше через ENV)
...
function log(...a){ console.log("[sphere]", ...a); }

// ====== простенькие cookie-сессии ===========================================
function sign(obj){
...
function readBody(req){
  return new Promise(resolve=>{
    let raw=""; req.on("data",d=>raw+=d); req.on("end",()=>resolve(raw));
  });
}
...
// ====== «БД» в памяти процесса ==============================================
// users[telegramId] = { id, name, handle, photoDataUrl?, spheres:[{id,name,url}] }
const users = Object.create(null);

// ====== проксирование =======================================================
function stripIframeHeaders(h){
...
function proxyGET(targetUrl, req, res){
  ...
  const up = mod.request({
    ...
    res.writeHead(u.statusCode||200, stripIframeHeaders(u.headers));
    u.pipe(res);
  });
  up.on("error", e=>{
...
<plist version="1.0"><dict>
 ...
 <key>PayloadVersion</key><integer>1</integer>
    ...
    // ===== API: auth/profile/spheres =======================================
    setCORS(res);

    // /api/me — текущее состояние
    if (req.url.startsWith("/api/me")){
      const sid = verify(getCookies(req).sid);
      const u = sid ? users[sid.id] : null;
    ...
    if (req.url.startsWith("/auth/telegram")){
      ...
      if (!users[user.id]) users[user.id] = { id:user.id, name:"", handle:"", photo:"", spheres:[] };
      setCookie(res,"sid", sign({id:user.id}));
      res.writeHead(302,{ "Location": "/app" });
      return res.end();
    }

    // /api/name — сохранить профиль (имя/handle/фото)
    if (req.url.startsWith("/api/name") && req.method==="POST"){
      const sid = verify(getCookies(req).sid);
      if (!sid || !users[sid.id]){ res.writeHead(401); return res.end("unauth"); }
      let body={}; try{ body=JSON.parse(await readBody(req)||"{}"); }catch{}
      // проверим уникальность handle
      const handle = String(body.handle||"").trim();
      if (!/^([a-zA-Z0-9_.]{3,20})$/.test(handle)){ res.writeHead(400); return res.end("bad handle"); }
      for (const uid of Object.keys(users)){
        if (uid!==sid.id && users[uid].handle===handle){ res.writeHead(409); return res.end("handle taken"); }
      }
    ...
    // /api/spheres — список/добавление/удаление
    if (req.url.startsWith("/api/spheres")){
      ...
      if (req.method==="POST"){
        ...
        // запретим дубликаты по имени
        if ((u.spheres||[]).some(s=>s.name.toLowerCase()===name.toLowerCase())){
          res.writeHead(409); return res.end("duplicate");
        }
        const s = { id: crypto.randomUUID(), name, url };
        u.spheres.push(s);
    ...
    // /api/ip — для отладки (через upstream proxy)
    if (req.url.startsWith("/api/ip")){
      ...
      r.on("error",()=>{ res.writeHead(200,{"Content-Type":"text/plain"}); res.end("unknown"); });
      r.end(); return;
    }
...
server.listen(PORT, ()=>log("listening on", PORT, "via", UPSTREAM_PROXY || "NO_PROXY"));
