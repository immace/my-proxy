/* SPHERE front-end — only Telegram login; unique spheres; no red dot on main;
   emulator header: [app name] | SPHERE | red dot; IP via /api/ip (through upstream proxy).
*/
...
(() => {
  ...
  *{box-sizing:border-box}
  ...
  document.body.innerHTML =
    '<div class="titlebar"><div></div><div class="brand">SPHERE</div><div></div></div>' + // без красной точки
    '<div class="stage" id="view-login">' +
    '  <div class="grid-bg" id="grid-login"></div>' +
    '  <div class="card">' +
    '    <div class="chip">Добро пожаловать</div>' +
    '    <h1>Sphere</h1>' +
    '    <p style="margin:0 0 8px;color:var(--muted)">Войдите через Telegram. Затем укажите имя/ник и (по желанию) загрузите аватар.</p>' +
  ...
  // ===== Telegram widget
  const BOT_NAME = (window.BOT_NAME || "").trim();
  if (BOT_NAME) {
    ...
    $("#tg-slot").innerHTML = '<div style="color:#ffb0b0">TG_BOT_NAME не задан на сервере</div>';
  }

  // позвать /api/me — если уже логин, либо ждём callback и просим имя/ник
  fetch("/api/me", { credentials:"include" })
    .then(r => r.json())
    .then(me => {
      if (me && me.id) { // есть сессия TG
        state.me = me;
        // имя/ник уже сохранены?
        const meLocal = JSON.parse(localStorage.getItem('sphere_me') || "{}");
        if (meLocal && meLocal.handle) { state.me = {...me, ...meLocal}; bootScene(); }
        else { $('#after-tg').style.display = 'block'; }
      } else {
        // ждём, что пользователь нажмёт виджет TG — после редиректа снова придём сюда
      }
    })
    .catch(()=>{});
  ...
  $('#finish').onclick = () => {
    ...
    state.me = { ...(state.me||{}), name, handle };

    const file = $('#avatar').files[0];
    if (file) {
      const r = new FileReader();
      r.onload = e => { state.me.photo = e.target.result; persistMe(); bootScene(); };
      r.readAsDataURL(file);
    } else { persistMe(); bootScene(); }
  };

  function persistMe(){ localStorage.setItem('sphere_me', JSON.stringify(state.me||{})); }
  ...
  // плюс-сфера + рендер окружности
  function mountPlus(){ $('#plusSphere').onclick = () => $('#dlg').showModal(); }

  function renderExistingSpheres() {
    $$('.sphere.added').forEach(el => el.remove());
    const R = 170, ring = $('#ring'); let i = 0;
    state.profiles.forEach(p => {
      const s = document.createElement('div'); s.className = 'sphere added';
      const ang = i * (Math.PI/5.5) + Math.PI/8; i++;
      ...
      s.title = p.name; s.onclick = () => openEmu(p);
      ring.appendChild(s);
    });
  }

  function chooseNet(net) {
    // запретить дубликаты по id сети
    if (state.profiles.some(p => p.netId === net.id)) {
      alert('Эта сфера уже добавлена'); $('#dlg').close(); return;
    }
    $('#dlg').close();
    ...
    fetch('/api/profile', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload)})
      .then(r=>r.ok?r.json():payload).then(p=>{
        const profile = { id: p.pid || crypto.randomUUID(), pid:p.pid, netId:net.id, handle:(state.me&&state.me.handle)||'', name:p.name, url:p.url };
        state.profiles.push(profile); save(); renderExistingSpheres(); openEmu(profile);
      }).catch(()=>{
        const profile = { id: crypto.randomUUID(), netId:net.id, handle:(state.me&&state.me.handle)||'', name:net.name, url:net.url };
        state.profiles.push(profile); save(); renderExistingSpheres(); openEmu(profile);
      });
  }
  ...
  function openEmu(p) {
    const ensure = p.pid
      ...
      const header =
        '<div style="height:56px;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;padding:10px 12px;border-bottom:1px solid var(--line);background:#0d1014">' +
        '  <div style="font-weight:700">'+ (p.name || '') +'</div>' +
        '  <div class="brand">SPHERE</div>' +
        '  <div style="justify-self:end"><div class="macdot"></div></div>' +
        '</div>';

      panel.innerHTML = header +
        '<div style="padding:10px;display:flex;gap:8px;border-bottom:1px solid var(--line);background:#0d1014">' +
      ...
      $('#resetBtn', panel).onclick = () => fetch('/api/reset?pid=' + encodeURIComponent(pr.pid || p.id), {method:'POST'}).then(()=>alert('Очищено')).catch(()=>alert('Очищено (локально)'));
      $('#ipBtn', panel).onclick = () => fetch('/api/ip?pid=' + encodeURIComponent(pr.pid || p.id)).then(r=>r.text()).then(t=>alert('IP профиля: ' + t)).catch(()=>alert('IP недоступен'));
  ...
  // ===== parallax mouse / deviceorientation
  function setupParallax() {
    ...
    window.addEventListener('mousemove', e => {
      const rx = (e.clientX/innerWidth - .5)*8, ry = (.5 - e.clientY/innerHeight)*6; tilt(rx,ry);
    }, {passive:true});
    if (typeof DeviceOrientationEvent !== 'undefined' && DeviceOrientationEvent.requestPermission) {
      window.addEventListener('click', ask, {once:true}); function ask(){
        DeviceOrientationEvent.requestPermission().then(s=>{ if(s==='granted') window.addEventListener('deviceorientation', dev, {passive:true}); }).catch(()=>{});
      }
    } else if (typeof DeviceOrientationEvent !== 'undefined') {
      window.addEventListener('deviceorientation', dev, {passive:true});
    }
    function dev(ev){ const rx=(ev.gamma||0)/8, ry=-(ev.beta||0)/10; tilt(rx,ry); }
  }

  // автозапуск сцены если уже всё заполнено ранее (sid живой + локальные имя/ник)
  try {
    const meRaw = localStorage.getItem('sphere_me'); if (meRaw) state.me = JSON.parse(meRaw);
  } catch {}
  // (основная логика запуска — в /api/me выше)
})();
